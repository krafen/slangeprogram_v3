...
def process_and_add_hose(
    ...
    # Single-coupling detection: second_row2 is None means only one coupling
    # was in the summary. Use Antall=2 for that coupling and mirror it into
    # second_row2 so the hylse/gsm logic below still works correctly.
...
def render_excel_batch_mode(df1, df2_all, mont_df, trykktest_df, prikling_df, get_cert_row):
    ...
    for _, row in import_df.iterrows():
        ...
        # Single-coupling detection, same rule as Quick/Full mode
...
def render_quick_mode(df1, df2_all, mont_df, trykktest_df, prikling_df, get_cert_row):
    ...
    with c1:
        first_line = st.text_input(
            "Slangebeskrivelse (Bindestreker må være med 😒)",
            placeholder="Slange/Lengde/Kupling 1/Kupling 2",
            key="quick_first_line",
        )
    with c2:
